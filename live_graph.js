...
export class LiveGraph {
    ...
    if (this.recording) {
      ...
      if (driveIntegration.isConnected() && this.data.length > 0) {
        try {
          ...
          await driveIntegration.uploadFile(csv, filename, folderPath);
          
          this.hideUploadProgress();
          console.log('Live graph data uploaded to drive');
        } catch (error) {
          this.hideUploadProgress();
  ...
  async saveCSV() {
    ...
    txtData += this.data.map(row => row.join('\t') + '\t' + (driveIntegration.getDriveLink() || '')).join('\n');
